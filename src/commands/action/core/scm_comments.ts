...
export class Comments {
  ...
  static removeAlerts(
    ...
    const [ignoreAll, ignoreCommands] = Comments.getIgnoreOptions(comments)
  ...
  static isHeadingLine(line: string): boolean {
    return (
  ...
  static processSecurityComment(
    ...
    for (const line of comment.bodyList) {
      ...
      if (trimmedLine.includes('start-socket-alerts-table')) {
        ...
        !Comments.isHeadingLine(trimmedLine) &&
        trimmedLine !== ''
      ) {
        ...
        const ignore =
          ...
          ignoreCommands.some(([name, version]) =>
            Comments.isIgnore(fullName, pkgVersion, name, version)
          )
