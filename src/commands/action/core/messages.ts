...
export class Messages {
  static createSecurityCommentJson(diff: Diff): Record<string, any> {
  ...
  static securityCommentTemplate(diff: Diff): string {
    ...
    const { mdUpdated, ignoreCommands, nextSteps } =
      Messages.createSecurityAlertTable(diff, md)
    md.push('<!-- end-socket-alerts-table -->')
    md.push('')
    Messages.createNextSteps(mdUpdated, nextSteps)
    Messages.createDeeperLook(mdUpdated)
    Messages.createRemovePackage(mdUpdated)
    Messages.createAcceptableRisk(mdUpdated, ignoreCommands)
    return md.join('\n')
  }

  static createNextSteps(
    md: string[],
  ...
  static createDeeperLook(md: string[]): void {
    md.push('<details>')
  ...
  static createRemovePackage(md: string[]): void {
    md.push('<details>')
  ...
  static createAcceptableRisk(md: string[], ignoreCommands: string[]): void {
    md.push('<details>')
  ...
  static createSecurityAlertTable(
    diff: Diff,
    md: string[]
  ): {
    mdUpdated: string[]
  ...
  static createSources(
    ...
    for (const [source, manifest] of alert.introducedBy) {
      ...
      const manifestStr =
        style === 'md' ? `<li>${manifest}</li>` : `${manifest};`
  ...
  static createDependencyOverviewTemplate(diff: Diff): string {
    const md = []
    ...
    md.push('')
    Messages.createAddedTable(diff, md)
    if (diff.removedPackages.length > 0) {
      Messages.createRemoveLine(diff, md)
    }
    return md.join('\n')
  }

  static createRemoveLine(diff: Diff, md: string[]): void {
    const removedLine = diff.removedPackages
      .map(removed => Messages.createPurlLink(removed))
      .join(', ')
    md.push(`Removed packages: ${removedLine}`)
  }

  static createAddedTable(diff: Diff, md: string[]): void {
    const headers = [
      'Package',
      'Direct',
      'Capabilities',
    ...
    const rows = diff.newPackages.map(added => {
      ...
      return `${Messages.createPurlLink(added)} | ${added.direct} | ${capabilities} | ${
        added.transitives
  ...
  static createPurlLink(details: Purl): string {
    return `[${details.purl}](${details.url})`
  }

  static createConsoleSecurityAlertTable(diff: Diff): chalkTable {
    // TODO: fix
    const alertTable = []
    const options = {
      columns: [
    ...
    for (const alert of diff.newAlerts) {
      // @ts-ignore
      const { manifestStr, sourceStr } = Messages.createSources(
        alert,
        'console'
      )
