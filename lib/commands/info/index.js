...
import { formatSeverityCount, getSeverityCount } from '../../utils/format-issues.js'
 ...
 */
async function fetchPackageData (ecosystem, pkgName, pkgVersion, { includeAllIssues }, spinner) {
  const socketSdk = await setupSdk(getDefaultKey() || FREE_API_KEY)
  // @ts-ignore
  const result = await handleApiCall(socketSdk.batchPackageFetch(
    { license: 'true', alerts: 'true' },
    {
        components:
            [{
  ...
  const alerts = await handleApiCall(socketSdk.getIssuesByNPMPackage(pkgName, pkgVersion), 'looking up package')
  ...
  if (!alerts.success) {
    return handleUnsuccessfulApiResponse('getIssuesByNPMPackage', alerts, spinner)
  }

  const severityCount = getSeverityCount(alerts.data, includeAllIssues ? undefined : 'high')
