...
export const info2 = {
  ...
  async run (argv, importMeta, { parentName }) {
    ...
    if (input) {
      ...
      const packageData = await fetchPackageData(input.ecosystem, input.pkgName, input.pkgVersion, spinner)
      if (packageData) {
 ...
 * @returns {void}
 */
function formatPackageDataOutput (/** @type {{ [key: string]: any }} */ data, { name, outputJson, outputMarkdown, pkgName, pkgVersion }, spinner) {
  if (outputJson) {
