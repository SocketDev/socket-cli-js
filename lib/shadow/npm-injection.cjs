...
async function * batchScan (
  ...
  const pkgDataReq = http.request(
    'http://localhost:8866/v0/scan/batch',
    {
      method: 'POST',
      headers: {
        Authorization: `Basic ${Buffer.from('sktsec_jp5V1O5Cn5-sJLSFwprzpNOCFNNMpbxFoF94q5FBzomp_api:').toString('base64url')}`
      }
    }
  ).end(
...
/**
 * @param {string[]} pkgs
 * @param {import('@socketsecurity/sdk').SocketSdk} _socketSdk
 * @param {import('ora')['default']} ora
 * @returns {Promise<boolean>}
 */
async function packagesHaveRiskyIssues (pkgs, _socketSdk, ora) {
  let failed = false
  if (pkgs.length) {
    ...
    const pkgDatas = []
    try {
      for await (const pkgData of batchScan(pkgs)) {
        const failures = []
