...
class SafeArborist extends Arborist {
  ...
  async reify (...args) {
    ...
    if (isInteractive) {
      ...
      const risky = await packagesHaveRiskyIssues(this.registry, diff.check, ora)
      if (risky) {
        const rl = require('readline')
      ...
      await packagesHaveRiskyIssues(this.registry, diff.check)
 ...
 */
async function packagesHaveRiskyIssues (registry, pkgs, ora = null) {
  let failed = false
  if (pkgs.length) {
    const auditProm = getNPMAudit(registry, pkgs)
    ...
    try {
      for await (const pkgData of batchScan(pkgs.map(pkg => pkg.pkgid))) {
        ...
        if (failures.length) {
          ...
          console.error(`(socket) ${formatter.hyperlink(`${name}@${version}`, `https://socket.dev/npm/package/${name}/overview/${version}`)} contains risks:`)
          if (translations) {
      ...
      if (spinner) {
        spinner.text = 'gathering new dependency audit from npm'
      }
      const auditResult = await auditProm
      for (const [name, risks] of Object.entries(auditResult)) {
        spinner?.stop()
        if (risks.length) {
          failed = true
          formatter ??= new ((await chalkMarkdownPromise).ChalkOrMarkdown)(false)
          console.log(`(npm) ${formatter.hyperlink(name, `https://npmjs.com/package/${name}`)} contains risks:`)
          for (const risk of risks) {
            console.error(`  ${risk.severity} - ${formatter.hyperlink(risk.title, risk.url)}`)
          }
        }
        spinner?.start()
      }
...
/**
 *
 * @param {URL['href']} registry
 * @param {InstallEffect[]} diff
 * @returns {Promise<Record<string, {severity: string, title: string, url: string}[]>>}
 */
async function getNPMAudit (registry, diff) {
  /**
   * @type {Record<string, string[]>}
   */
  const body = {}
  for (const c of diff) {
    const { name, version } = pkgidParts(c.pkgid)
    const versions = body[name] ??= []
    versions.push(version)
  }
  const req = https.request(`${registry}/-/npm/v1/security/advisories/bulk`, {
    method: 'POST'
  })
  ...
  await new Promise((resolve, reject) => {
    ...
    events.once(res, 'end').then(resolve, reject)
  })
  return JSON.parse(Buffer.concat(resBody).toString('utf-8'))
}
