...
class SafeArborist extends Arborist {
   ...
   */
  [kRiskyReify] (...args) {
    return new Arborist(this[kCtorArgs]).reify(...args)
  }
  ...
  async reify (...args) {
    ...
    // @ts-ignore
    await super.reify(mutedArguments, ...args.slice(1))
    // @ts-ignore
    ...
    if (diff.check.length === 0 && diff.unknowns.length === 0) {
      console.log('SAFE DIFF', args, diff)
      return this[kRiskyReify](...args)
    }
    ...
    try {
      for (const pkg of pkgs) {
        ...
        if (failures.length) {
          ...
          console.error(`${formatter.hyperlink(pkg, `https://socket.dev/npm/package/${name}/overview/${version}`)} contains issues:`)
          if (translations) {
