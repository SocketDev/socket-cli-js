...
class SafeArborist extends Arborist {
  ...
  async reify (...args) {
    ...
    return ttyServer.captureTTY(async (input, output, colorLevel) => {
      if (input) {
        ...
        if (risky) {
          ...
          const ret = this[kRiskyReify](...args)
          return await ret
        }
      } else {
        ...
        return new Promise((resolve, reject) => {
          ...
          conn.on('data', d => {
            if (!captured) {
              ...
              if (EOL !== -1) {
                captured = true
                const { capabilities: { input: hasInput, output: hasOutput, colorLevel: ipcColorLevel } } = JSON.parse(d.slice(0, EOL).toString('utf-8'))
                const input = hasInput ? new PassThrough() : null
                input.pause()
                // conn.push(d.slice(EOL + 1))
                conn.pipe(input)
